...
impl Pdf {
    ...
    fn compress(&self, quality: u8) -> Vec<(Vec<u8>, u32, u32)>{
        // Get widths
        let widths: Vec<u32> = self.imgs.par_iter()
    ...
    pub fn create(&self, output: String, quality: u8) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (data, width, height) in img_data.iter() {
            ...
            let x_pt: i32 = (x_cm as f64 * 0.3528) as i32;
            let y_pt: i32 = (y_cm as f64 * 0.3528) as i32;
            ...
            let content = Content {
                operations: vec![
                    Operation::new("q", vec![]), 
                    Operation::new("cm", vec![Object::Integer(x_pt as i64), Object::Integer(0), Object::Integer(0), Object::Integer(y_pt as i64), Object::Integer(0), Object::Integer(0)]),
            ...
            let page_id = doc.add_object(dictionary! {
                ...
                "Contents" => content_id,
                "Resources" => resources,
                "MediaBox" => vec![0.into(), 0.into(), x_pt.into(), y_pt.into()],
            });
            num_pages += 1;
            kids.push(page_id);
        }
